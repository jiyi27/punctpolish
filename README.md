...
- 保留 Markdown fenced code block 原样不动
...
```bash
./punctpolish --file /path/to/note.md
```
...
- `--debounce`: 覆盖默认 debounce 时间 (仅 `--dir` 模式有效)
...
- Markdown fenced code block 中的内容会原样保留, 不参与替换
- 程序不会主动压缩多空格, 以免破坏逻辑树、手工对齐文本等结构
- Markdown 图片/链接语法以及裸 `http(s)` 链接会尽量保留原样, 避免路径或 URL 因自动加空格而失真
